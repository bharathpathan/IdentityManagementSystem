...
public class HibernateIdentityDAO {
	...
	public Collection<Identity> search(Identity criteria) {
		...
		final Session session = sessionFactory.openSession();
        identitiesList=(List<Identity>)session.createQuery("from Identity WHERE IDENTITY_DISPLAY_NAME='"+criteria.getDisplayName() +"' AND IDENTITY_EMAIL='"+criteria.getEmail() +"'",Identity.class).list();
